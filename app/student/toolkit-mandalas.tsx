...
import { Canvas, Group, Path, Skia, Paint, BlendMode } from '@shopify/react-native-skia';
...
interface PathData {
  ...
  color: string;
  strokeWidth: number;
  offsetX?: number; // optional translation X
  offsetY?: number; // optional translation Y
}

export default function MandalaToolkit() {
  // eraser blend mode setup
  ...
  const canvasWidth = screenWidth - 40; // or the width of your canvas container
  const canvasHeight = canvasWidth;     // if square
  const templateCenterX = 100;          // adjust based on template path
  const templateCenterY = 100;
  ...
  const addTemplateToCanvas = () => {
    ...
    const canvasWidth = screenWidth - 40; // match your canvas container width
    const canvasHeight = canvasWidth;     // if canvas is square

    // Approximate template center
    const templateCenterX = 100; // adjust based on your template coordinates
    const templateCenterY = 100;

    // Compute translation to center the template
    const offsetX = canvasWidth / 2 - templateCenterX;
    const offsetY = canvasHeight / 2 - templateCenterY;

    // Create a new path string with translation applied
    // In Skia, we can use a Group transform, but for simplicity here we'll store it as-is
    // We can just save the original path and apply transform when rendering
    const centeredPathData = {
    ...
    // Add to paths
    setPaths(prev => [...prev, centeredPathData]);

    // Deselect template
    setSelectedTemplate(null);
  };

  function doesPathIntersect(path1: string, path2: string, threshold: number) {
  // naive approach: if bounding boxes overlap
  const bbox1 = getPathBounds(path1);
  const bbox2 = getPathBounds(path2);

  return !(
    bbox2.x + bbox2.width + threshold < bbox1.x ||
    bbox2.x > bbox1.x + bbox1.width + threshold ||
    bbox2.y + bbox2.height + threshold < bbox1.y ||
    bbox2.y > bbox1.y + bbox1.height + threshold
  );
}

function getPathBounds(path: string) {
  // parse "M x,y L x,y ..." into coords
    ...
    <View style={styles.container}>
        ...
        >
          <Canvas style={StyleSheet.absoluteFillObject}>
            ...
            {selectedTemplate && (
              <Group
                transform={[
                  { translateX: canvasWidth / 2 }, // move to canvas center X
                  { translateY: canvasHeight / 2 }, // move to canvas center Y
                  { translateX: -templateCenterX },  // shift path center to origin
                  { translateY: -templateCenterY },
                ]}
              >
                <Path
                  path={selectedTemplate.path}
            ...
            {paths.map((pathData, index) => (
              <Path
                ...
                transform={[
                  { translateX: pathData.offsetX || 0 },
                  { translateY: pathData.offsetY || 0 }
                ]}
              />
            ))}
            ...
            {currentPath && (
              <Path
                ...
                style="stroke"
                strokeWidth={isEraser ? eraserSize : strokeWidth}
                color={isEraser ? '#000000' : strokeColor} // color is ignored when blendMode=clear
                paint={isEraser ? eraserPaint : undefined}
              />
            )}
          </Canvas>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <Button title="Select template" onPress={() => { addTemplateToCanvas() }} />
